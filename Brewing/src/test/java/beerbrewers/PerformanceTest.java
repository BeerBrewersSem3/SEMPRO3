...
public class PerformanceTest {
    ...
    @Autowired
    PerformanceTest(MachineService machineService) {
       this.worker = new Worker(1L,"John John", "MyPassword");
       this.operation= new Operation(1L, worker);
       this.batch = new Batch(1L, operation, Brew.IPA, 100L, 100L);
    ...
    public void testStartMachine() {
        ...
        for (int i = 1; i <= 100; i++) {
            ...
            this.machineService.setBatchAttributesToMachine(this.batch);
            this.machineService.startMachine();
            long endTime = System.currentTimeMillis();
            this.machineService.resetMachineState();
